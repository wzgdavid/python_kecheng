...
for i in range(length):
    if y_pred[i] == types[0]:
        type0[0].append(df2.F[i])
        type0[1].append(df2.M[i])
    elif y_pred[i] == types[1]:
        type1[0].append(df2.F[i])
        type1[1].append(df2.M[i])
    elif y_pred[i] == types[2]:
        type2[0].append(df2.F[i])
        type2[1].append(df2.M[i])
    elif y_pred[i] == types[3]:
        type3[0].append(df2.F[i])
        type3[1].append(df2.M[i])
print(type0)
